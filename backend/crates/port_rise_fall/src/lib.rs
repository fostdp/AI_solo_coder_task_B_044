use maritime_common::config::{MaritimeInsightsConfig, PanelRegressionConfig, GrangerCausalityConfig};
...
pub(crate) fn build_design_matrix(points: &[PanelDataPoint]) -> (Vec<Vec<f64>>, Vec<f64>) {
    let n = points.len();
...
pub(crate) fn run_panel_regression_for_port(
    port_id: i32,
    port_name: &str,
    flows: &[PortYearlyFlow],
    climate_map: &HashMap<i32, ClimatePeriod>,
...
pub(crate) fn run_granger_tests_for_port(
    port_id: i32,
    port_name: &str,
    flows: &[PortYearlyFlow],
    climate_map: &HashMap<i32, ClimatePeriod>,
...
pub(crate) fn fill_missing_with_fallback(points: &mut [PanelDataPoint]) {
    if points.is_empty() {
        return;
    }
...
pub mod regression_service {
    ...
    use crate::{PanelRegression, build_panel_data, build_design_matrix, fill_missing_with_fallback, run_panel_regression_for_port, run_granger_tests_for_port};

    pub struct RegressionRequest {
        pub port_id: i32,
        pub port_name: String,
        pub flows: Vec<PortYearlyFlow>,
        pub climate_map: HashMap<i32, ClimatePeriod>,
        pub war_counts: HashMap<i32, i32>,
        pub regime_counts: HashMap<i32, i32>,
    }

    pub struct RegressionResponse {
        pub port_id: i32,
        pub regression: Option<PanelRegressionResult>,
        pub granger: Vec<GrangerCausalityResult>,
    }

    pub fn start_regression_worker(config: PanelRegressionConfig) -> mpsc::SyncSender<RegressionRequest> {
        let (tx, rx): (mpsc::SyncSender<RegressionRequest>, mpsc::Receiver<RegressionRequest>) = mpsc::sync_channel(32);
        std::thread::spawn(move || {
            let cfg = MaritimeInsightsConfig {
                panel_regression: config,
                granger_causality: maritime_common::config::GrangerCausalityConfig::default(),
                route_planning: maritime_common::config::RoutePlanningConfig::default(),
                cargo_spread: maritime_common::config::CargoSpreadConfig::default(),
                modern_comparison: maritime_common::config::ModernComparisonConfig::default(),
                port: 0,
                metrics_port: 0,
            };
            while let Ok(req) = rx.recv() {
                let reg = run_panel_regression_for_port(
                    req.port_id, &req.port_name, &req.flows,
                    &req.climate_map, &req.war_counts, &req.regime_counts, &cfg,
                );
                let granger = run_granger_tests_for_port(
                    req.port_id, &req.port_name, &req.flows,
                    &req.climate_map, &req.war_counts, &req.regime_counts, &cfg,
                );
                let _ = (reg, granger);
...
mod tests {
    ...
    mod regression_service_tests {
        ...
        #[test]
        fn test_regression_worker_channel_creation() {
            let config = PanelRegressionConfig::default();
            let tx = regression_service::start_regression_worker(config);
            drop(tx);
        }

        #[test]
        fn test_regression_request_send() {
            let config = PanelRegressionConfig::default();
            let tx = regression_service::start_regression_worker(config);
            let req = regression_service::RegressionRequest {
                port_id: 1,
                port_name: "TestPort".to_string(),
                flows: vec![],
                climate_map: HashMap::new(),
                war_counts: HashMap::new(),
                regime_counts: HashMap::new(),
            };
            let result = tx.send(req);
            assert!(result.is_ok());
            drop(tx);
        }

        #[test]
        fn test_regression_request_drop_closes_worker() {
            let config = PanelRegressionConfig::default();
            let tx = regression_service::start_regression_worker(config);
            drop(tx);
        }
