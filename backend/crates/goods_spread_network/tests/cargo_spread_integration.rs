...
use maritime_common::models::{Port, VoyageRecord};
...
fn test_cross_civilization_routes_identification() {
    ...
    let cross_routes = identify_cross_civilization_routes(&voyages, &ports);
...
fn test_build_cargo_spread_network_basic() {
    ...
    let network = build_cargo_spread_network(&voyages, &ports, "spices", 10);

    assert!(network.nodes.len() >= 2);
    assert!(network.edges.len() >= 1);
    assert!(!network.origin_ports.is_empty());
}
