...
pub mod thread_pool {
    ...
    pub fn spawn_route_thread(config: RoutePlanningConfig) -> mpsc::SyncSender<RouteRequest> {
        let (tx, rx): (mpsc::SyncSender<RouteRequest>, mpsc::Receiver<RouteRequest>) = mpsc::sync_channel(16);
        std::thread::spawn(move || {
            let _config = config;
            while let Ok(_req) = rx.recv() {
                // Route computation handled by async plan_optimal_route via tokio::runtime
            }
        });
        tx
    }
    ...
    pub async fn compute_route_async(
        ...
        season: &str,
        ship_type: &str,
    ) -> Option<crate::RoutePlanningResult> {
        tokio::task::spawn_blocking(move || {
            // The actual computation is async, so we use a new runtime
            let rt = tokio::runtime::Runtime::new().unwrap();
            rt.block_on(async {
                crate::plan_optimal_route(pool, config, dep_id, arr_id, season, ship_type).await
            })
        }).await.ok().flatten()
    }
}
...
mod tests {
    ...
    mod thread_pool_tests {
        ...
        #[test]
        fn test_route_thread_spawn() {
            let config = RoutePlanningConfig::default();
            let tx = thread_pool::spawn_route_thread(config);
            drop(tx);
        }

        #[test]
        fn test_route_request_send() {
            let config = RoutePlanningConfig::default();
            let tx = thread_pool::spawn_route_thread(config);
            let req = thread_pool::RouteRequest {
                departure_port_id: 1,
                arrival_port_id: 2,
                season: "summer".to_string(),
                ship_type: "trireme".to_string(),
        ...
        #[test]
        fn test_route_thread_multiple_requests() {
            let config = RoutePlanningConfig::default();
            let tx = thread_pool::spawn_route_thread(config);
            for i in 0..5 {
                let req = thread_pool::RouteRequest {
                    departure_port_id: i,
                    arrival_port_id: i + 10,
                    season: "winter".to_string(),
                    ship_type: "dhow".to_string(),
                };
                assert!(tx.send(req).is_ok());
            }
            drop(tx);
        ...
        #[test]
        fn test_route_request_drop_closes_thread() {
            let config = RoutePlanningConfig::default();
            let tx = thread_pool::spawn_route_thread(config);
            drop(tx);
        }
