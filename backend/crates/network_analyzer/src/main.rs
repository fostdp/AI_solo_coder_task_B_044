...
async fn main() {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(std::env::var("RUST_LOG").unwrap_or_else(|_| {
            "network_analyzer=info,tower_http=info,axum=info".to_string()
        }))
        .with_target(true)
        .with_level(true)
        .json()
    ...
    let prometheus_port = config.network_analyzer.port + 1000;
    PrometheusBuilder::new()
        .with_http_listener(([0, 0, 0, 0], prometheus_port))
        .install()
        .expect("Failed to install Prometheus exporter");

    counter!("network_analyzer_startups_total").increment(1);
    ...
    tracing::info!(
        port = config.network_analyzer.port,
        metrics_port = prometheus_port,
        "NetworkAnalyzer service starting"
    );
