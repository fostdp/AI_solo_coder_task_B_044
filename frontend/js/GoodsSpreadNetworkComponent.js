/**
 * 货物传播与文化交流网络模块
 * 对应后端 crate: goods_spread_network
 */

const GoodsSpreadNetworkComponent = (function () {
    ...
    async function analyze(cargoType) {
        showLoading();
        try {
    ...
    function clear() {
        stopTechDiffusionAnimation();
        currentTechDiffusion = null;

        if (cargoSpreadLayer) {
            cargoSpreadLayer.clearLayers();
        }
        if (techDiffusionLayer) {
            techDiffusionLayer.clearLayers();
        }
        if (map) {
            if (map.hasLayer(cargoSpreadLayer)) {
                map.removeLayer(cargoSpreadLayer);
            }
            if (map.hasLayer(techDiffusionLayer)) {
                map.removeLayer(techDiffusionLayer);
            }
        }
        ...
        AppState.cargoSpreadData = null;
        AppState.layers.cargoSpread = false;

        const layerCheckbox = document.getElementById('layer-cargo-spread');
        if (layerCheckbox) layerCheckbox.checked = false;

        const infoEl = document.getElementById('cargo-spread-info');
        if (infoEl) infoEl.innerHTML = '';

        const techInfoEl = document.getElementById('tech-diffusion-info');
        if (techInfoEl) techInfoEl.innerHTML = '';
    }
...
window.GoodsSpreadNetworkComponent = GoodsSpreadNetworkComponent;
